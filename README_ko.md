...
축은 식을 읽습니다. 삼각함수에는 π 눈금이 붙고 `exp`에는 붙지 않습니다. 축은 *창*도
읽는데, 이건 없을 때에야 알아차리게 되는 부분입니다 — 확대하면 처음 그릴 때 정해진
자리에 라벨을 그대로 두지 않고 다시 맞춥니다. π의 배수가 아직 들어맞는 동안에는 더
촘촘한 배수로, 더 이상 들어맞지 않으면 일반 숫자로, 그리고 깊이 확대해 라벨이 길어지면
개수를 줄여서.

```python
plot(x*y, ticks=4)      # 직접 개수를 제한할 수도 있습니다 — 카메라를 당겨도 라벨을
                        # 다시 배치하지 않는 3D에서는 이것이 유일한 수단입니다
```
...
Jupyter에서 그냥 `plot()`을 쓰면 그림 옆에 라이브 범위 조절 사이드바가 함께
나옵니다. 둘 다 원하는 대로 바꿀 수 있습니다:

```python
plot(sin(x), height=800)          # 이 plot만 더 높게
plot(sin(x), controls=False)      # 이 plot만 사이드바 없이 — 셀 전체가 그래프
set_plot_size(height=720)         # 이후 모든 plot, 노트북 맨 위에서 한 번만
set_range_controls(False)         # 이후 모든 plot, 순수한 그림으로
```

높이 기본값은 Plotly의 450이 아니라 520px입니다 — 노트북 셀은 대시보드 타일이
아니고, 너비의 5분의 1은 사이드바가 가져갑니다. 너비는 그림이 셀을 가득 채우도록
일부러 지정하지 않았습니다. 고정 크기를 정말로 원할 때만 지정하세요.
...
v1.0의 모든 기능을 안내하는 노트북을 직접 실행해 보세요. 곡선, 불연속성, 3D, 슬라이더, `analyze()`, 표, 데이터 적합(fitting), 워크시트 및 거부 사례가 포함된 124개의 셀로 구성되어 있습니다. 네트워크 연결이 필요하지 않습니다.
...
튜토리얼, 안내서, 매뉴얼의 모든 `python` 예제는 테스트 제품군에 의해 문서 순서대로 실행되므로, 문서의 코드는 실제 동작과 일치함이 보장됩니다. PRD는 설계 기록이라 그 안의 코드 조각은 실행되지 않습니다.
