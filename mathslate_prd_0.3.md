...
the axis. The reduction in guarantees is reported in the notes (`"50% of the grid is not a real number"`), not glossed over.
...
Fixed at the source rather than in the message: `jupyter = ["ipywidgets>=8.0", "anywidget>=0.9"]`. One extra now installs everything `range_controls()`
...
*mechanism* that could not deliver it in general: `_RANGE_CONTROLS_SIDEBAR_ WIDTH` and `_RANGE_CONTROLS_FIGURE_WIDTH` were both **pixel** counts, chosen
...
own hand-written `_mesh_kwarg()`, parallel to `RenderOptions.surface_ contours()` rather than calling it, and it never emitted `size` at all —
...
parameters — Z detection and seeding are automatic. `tests/test_view_and_ mesh.py` gained `TestMeshDensity` (default spacing, an explicit count,
...
## 24. marimo's partial ipywidgets bridge does not carry `range_controls()`

A later investigation found an inviting fact in marimo 0.23.15's installed
source: `IPyWidgetsFormatter.register()` calls
`Widget.on_widget_constructed(init_marimo_widget)`, and that initializer gives
each constructed widget a `MarimoComm` with target name `"jupyter.widgets"`
and registers `w._handle_msg` for incoming messages. Read narrowly, that
looks exactly like the standard traitlets route `FloatText.observe()` needs.
The question was therefore worth testing rather than preserving §19.2's
restriction by habit: could `plot(...).range_controls()` become a one-cell
marimo form, with no native `mo.ui.number()` pair?

The headless answer was deliberately not accepted. Constructing the bundle
under a stubbed marimo frontend says only that Python can construct an HBox;
it says nothing about the browser being able to render or send messages for
it. The real check ran `marimo run examples/marimo_notebook.py` in Chromium,
with the single-cell expression as the output. A Plotly figure did render,
but marimo replaced the containing standard `ipywidgets.HBox` bundle with its
own visible message: **"Jupyter widgets are not supported in marimo. Please
migrate this widget to anywidget."** The `FloatText` controls were therefore
not available to edit, so no comm round-trip or resample could take place.

That apparent contradiction is the important distinction. marimo's backend
does create the comm and its frontend can render an anywidget-based
`FigureWidget`; it does not thereby implement the browser views for ordinary
ipywidgets layout and input widgets that make this controller a bundle.
`range_controls()` remains Jupyter/Colab-only, and its precise error continues
to direct marimo readers to two cells of `mo.ui.number()` plus `plot()`. The
native example remains the supported live-resampling route. This is a failed
extension, not a silent capability claim; the live-browser result, rather
than a constructor test, is the evidence that decides it.